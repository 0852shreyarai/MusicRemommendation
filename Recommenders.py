...
class item_similarity_recommender_py():
    ...
    def generate_top_recommendations(self, user, cooccurence_matrix, all_songs, user_songs):
        ...
        #Sort the indices of user_sim_scores based upon their value and maintain the corresponding score
        ...
        for i in range(0,len(sort_index)):
            if ~np.isnan(sort_index[i][0]) and all_songs[sort_index[i][1]] not in user_songs and rank <= 10:
                dataframe.loc[len(dataframe)]=[user,all_songs[sort_index[i][1]],sort_index[i][0],rank]
                rank = rank + 1
        
        #Handle the case where there are no recommendations
        if dataframe.shape[0] == 0:
            print("Sorry, the current user has no songs for training the item similarity based recommendation model.")
            return -1
        else:
            return dataframe
    ...
    def recommend(self, user):
        ...
        #3. Construct item cooccurence matrix of size 
        cooccurence_matrix = self.construct_cooccurence_matrix(user_songs, all_songs)
        
        #4. Use the cooccurence matrix to make recommendations
        dataframe_recommendations = self.generate_top_recommendations(user, cooccurence_matrix, all_songs, user_songs)
